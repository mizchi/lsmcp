...
const TOOLS_REGISTRY: ToolInfo[] = [
  ...
  },
  {
    name: "move_directory",
    description:
      "Move a directory and update all TypeScript imports and references",
    category: "typescript",
    requiresLSP: false,
  },
  {
  ...
  },
  {
    ...
    description:
      "Get detailed signature information for a specific type from a module",
    category: "typescript",
    requiresLSP: false,
  },
  {
  ...
  },
  {
    ...
    description:
      "Get all symbols visible at a specific location in a TypeScript/JavaScript file",
    category: "typescript",
    requiresLSP: false,
  },
  {
...
function formatToolsList(tools: ToolInfo[], category: string): string {
  ...
  if (
    typescript.length > 0 &&
    (category === "all" || category === "typescript")
  ) {
    result += "## 🔧 TypeScript Tools (Compiler API)\n";
    result +=
      "These tools use the TypeScript Compiler API directly and don't require an LSP server.\n\n";
