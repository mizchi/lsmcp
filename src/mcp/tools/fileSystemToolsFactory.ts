...
function createFileSystemAdapter(api: FileSystemApi) {
  return {
    existsSync: (path: string) => {
      // This is a limitation - we need to use sync API for gitignore
      // For now, we'll use node fs directly when FileSystemApi is nodeFileSystemApi
      if (api === nodeFileSystemApi) {
        return require("node:fs").existsSync(path);
      }
      // For other implementations, this won't work correctly with gitignore
      console.warn(
        "FileSystemApi other than nodeFileSystemApi may not work correctly with gitignore",
      );
      return false;
    },
    ...
    execute: async ({ relativePath, recursive, maxAnswerChars = 200000 }) => {
      try {
        ...
        const gitignoreFilter = createGitignoreFilter(
          rootPath,
          createFileSystemAdapter(fileSystemApi) as any,
        );
        ...
        async function scanDir(
          ...
          const entries = await fileSystemApi.readdir(dirPath);

          for (const entry of entries) {
            const fullPath = join(dirPath, entry);
            const relPath = relative(rootPath, fullPath);

            const stats = await fileSystemApi.stat(fullPath);
