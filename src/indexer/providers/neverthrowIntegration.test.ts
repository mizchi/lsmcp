...
describe("Neverthrow Library Integration", () => {
  ...
  beforeAll(() => {
    ...
    if (!existsSync(neverthrowPath)) {
      console.warn(
        "⚠️  Neverthrow not installed. Run: pnpm add neverthrow --save-dev",
      );
    }
  });
  ...
  describe("Import Parsing for Neverthrow", () => {
    it("should parse neverthrow imports correctly", () => {
      ...
      expect(imports[0].specifiers.map((s) => s.local)).toContain(
        "fromThrowable",
      );
      ...
      expect(imports[1].specifiers.map((s) => s.local)).toContain(
        "ResultAsync",
      );
      expect(imports[1].specifiers.map((s) => s.local)).toContain(
        "fromAsyncThrowable",
  ...
  describe("Symbol Statistics", () => {
    it("should count symbol occurrences in neverthrow", async () => {
      ...
      const stats = {
        ...
        fromAsyncThrowable: (content.match(/\bfromAsyncThrowable\b/g) || [])
          .length,
