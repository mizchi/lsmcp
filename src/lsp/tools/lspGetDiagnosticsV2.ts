...
async function getDiagnosticsWithLSPV2(
  ...
  debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Getting diagnostics for ${request.filePath}`);
  ...
  try {
    ...
    if (documentWasOpen && (request.forceRefresh !== false)) {
      debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", "Closing existing document for refresh");
    ...
    debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Opening document with language: ${languageId}`);
    ...
    try {
      debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", "Trying push diagnostics");
      ...
      debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Push diagnostics success: ${diagnostics.length} diagnostics`);
    } catch (error) {
      debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Push diagnostics failed: ${error}`);
      
      // Strategy 2: Try pull diagnostics
      try {
        debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", "Trying pull diagnostics");
        ...
        debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Pull diagnostics failed: ${pullError}`);
        
        // Strategy 3: Polling fallback
        debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", "Falling back to polling");
        method = "polling";
        ...
        for (let poll = 0; poll < maxPolls; poll++) {
          ...
          if (diagnostics.length > 0) {
            debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Polling success after ${poll + 1} attempts`);
            break;
          }
          
          // Force document update every few polls
          if (poll > 0 && poll % 3 === 0) {
            debugLogger.log(LogLevel.DEBUG, "DiagnosticsV2", `Forcing document update (poll ${poll})`);
            client.updateDocument(fileUri, fileContent, poll + 2);
          }
          
          // Check timeout
          if (Date.now() - startTime > timeout) {
            debugLogger.log(LogLevel.WARN, "DiagnosticsV2", "Polling timeout reached");
            break;
          }
        }
      }
    ...
    debugLogger.log(LogLevel.INFO, "DiagnosticsV2", 
    ...
    try {
      ...
      debugLogger.log(LogLevel.WARN, "DiagnosticsV2", `Cleanup error: ${cleanupError}`);
    }
    ...
    debugLogger.log(LogLevel.ERROR, "DiagnosticsV2", `Failed after ${totalTime}ms: ${errorMessage}`, undefined, error instanceof Error ? error : undefined);
    
    return err(`Diagnostics failed: ${errorMessage} (${totalTime}ms, ${attempts} attempts)`);
  }
}

export const lspGetDiagnosticsV2Tool = createLSPTool({
  name: "get_diagnostics_v2",
  description: "Get diagnostics (errors, warnings) for a file using enhanced LSP with debugging",
  schema,
  language: "lsp",
