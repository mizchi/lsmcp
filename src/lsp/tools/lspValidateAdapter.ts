...
import { pyrightAdapterV2 } from "../../adapters/pyrightV2.ts";
import { debugLogger, LogLevel } from "../debugLogger.ts";

const schema = z.object({
  root: z.string().describe("Root directory of the project"),
  adapter: z.string().optional().describe("Adapter to validate (default: current adapter)"),
  includeHealthCheck: z.boolean().optional().describe("Include health check (default: true)"),
  includeFeatureTest: z.boolean().optional().describe("Include feature tests (default: true)"),
});
...
async function validateAdapter(
  request: ValidateAdapterRequest,
): Promise<Result<ValidateAdapterSuccess, string>> {
  try {
    debugLogger.log(LogLevel.INFO, "AdapterValidator", `Validating adapter for ${request.root}`);

    // For now, only support pyright validation
    // TODO: Make this work with any adapter
    const adapter = pyrightAdapterV2;
    ...
    try {
      ...
      debugLogger.log(LogLevel.DEBUG, "AdapterValidator", "Checking adapter availability");
      
      result.availability = await adapter.checkAvailability();
      
      if (!result.availability.available) {
        debugLogger.log(LogLevel.WARN, "AdapterValidator", `Adapter not available: ${result.availability.message}`);
        return ok(result);
      }

      // Step 2: Health check
      if (request.includeHealthCheck !== false) {
        debugLogger.log(LogLevel.DEBUG, "AdapterValidator", "Running health check");
        ...
        result.health = {
          ...
          workingFeatures: health.validationResult.featureTests.filter(f => f.working).length,
          totalFeatures: health.validationResult.featureTests.length,
        };
      }

      // Step 3: Diagnostic info
      debugLogger.log(LogLevel.DEBUG, "AdapterValidator", "Getting diagnostic info");
      ...
      debugLogger.log(LogLevel.INFO, "AdapterValidator", "Validation completed successfully");
    ...
    debugLogger.log(LogLevel.ERROR, "AdapterValidator", `Validation failed: ${errorMessage}`, undefined, error instanceof Error ? error : undefined);
...
export const lspValidateAdapterTool = createLSPTool({
  ...
  formatSuccess: (result) => {
    ...
    // Availability
    lines.push(`\nAvailability: ${result.availability.available ? "✓ Available" : "✗ Not Available"}`);
    ...
    if (result.health) {
      ...
      lines.push(`  Connection: ${result.health.connectionSuccess ? "✓" : "✗"}`);
      lines.push(`  Initialization: ${result.health.initializationSuccess ? "✓" : "✗"}`);
      lines.push(`  Working Features: ${result.health.workingFeatures}/${result.health.totalFeatures}`);
