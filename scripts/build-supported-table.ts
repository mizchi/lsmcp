...
async function loadAdapters(): Promise<AdapterInfo[]> {
  ...
  for (const file of adapterFiles) {
    ...
    const adapterMatch = content.match(/export const (\w+Adapter): LspAdapter = \{([^}]+(?:\{[^}]*\}[^}]*)*)\}/s);
...
function generateMarkdownTable(adapters: AdapterInfo[]): string {
  ...
  md.push("<!-- GENERATED BY scripts/build-supported-table.ts - DO NOT EDIT MANUALLY -->");
  md.push("");
  md.push("# LSP Feature Support Table");
  md.push("");
  md.push("This table shows which LSP features are supported by each adapter.");
  md.push("Generated automatically by `scripts/build-supported-table.ts` - do not edit manually.");
  md.push("");
  ...
  md.push("");
  md.push("## Notes");
  md.push("- Features marked as ❌ will return appropriate error messages when called");
  md.push("- Some adapters may have partial support for certain features");
  md.push("- This table reflects the current implementation and may change as adapters are updated");
